...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const imagePath = path.join(__dirname, '../img/role-age.png');
            const attachment = new AttachmentBuilder(imagePath).setName('role-age.png');
    ...
    handleButtonInteraction: async (interaction) => {
        ...
        try {
            ...
            if (existingAgeRole) {
                console.log(`Retrait du rôle existant : ${existingAgeRole.name} (${existingAgeRole.id})`);
                await interaction.member.roles.remove(existingAgeRole);
                await interaction.editReply({
                    content: `Votre rôle précédent (${existingAgeRole.name}) a été retiré.`
                });
            } else {
                console.log('Aucun rôle d’âge existant trouvé.');
                await interaction.editReply({
                    content: 'Aucun rôle d’âge précédent à retirer.'
                });
            }
